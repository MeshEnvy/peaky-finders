"""Unified CLI: granular Make helpers, makefile generation, GDB inspect."""
...
from peaky_finders.generate_makefile_cli import (
    build_generate_makefile_parser,
    run_generate_makefile,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="peaky",
        description=(
            "Peaky Finders: generate Makefile from preset YAML, then ``make``. "
            "Granular peaky bundle/viewshed/mesh/stamp/kmz for recipes."
        ),
    )
    ...
    p_gen = sub.add_parser(
        "generate-makefile",
        parents=[build_generate_makefile_parser()],
        help="Emit configure-style Makefile for a preset job",
    )
    p_gen.set_defaults(_handler=run_generate_makefile)
    ...
    p_stamp = sub.add_parser(
        ...
        help="Write build/stamps sections for Makefile edges",
    )
    p_stamp.set_defaults(_handler=run_stamp)

    p_bundle = sub.add_parser("bundle", help="Granular bundle targets (Makefile recipes)")
    ...
    pdem.add_argument(
        ...
        metavar="N37W117",
        default=None,
        help="Fetch exactly this Skadi 1° cell (Makefile DEM_* targets)",
    )
    ...
    p_viewshed = sub.add_parser(
        ...
        help="Coverage run for one site slug (persisted workspace under preset build/viewsheds)",
    )
    ...
    p_kmz = sub.add_parser(
        ...
        help="Assemble aggregate KMZ from bundle + persisted viewshed/mesh artifacts (no SPLAT)",
    )
