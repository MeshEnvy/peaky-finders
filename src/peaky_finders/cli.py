"""Host CLI helpers: granular coverage (Makefile), KMZ aggregation."""
...
def build_granular_viewshed_argument_parser() -> argparse.ArgumentParser:
    """Flags for ``peaky viewshed`` (Makefile granular targets require ``--workspace-only --phase``)."""
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Run exactly one persisted-workspace phase (Makefile; requires --phase).",
    )
    p.add_argument(
        "--phase",
...
def run_splat(args: argparse.Namespace) -> int:
    """One viewshed workspace phase under ``build/viewsheds/`` — invoked by Makefile targets."""

    if not getattr(args, "viewshed_workspace_only", False):
        print(
            "`peaky viewshed` requires --workspace-only (Makefile granular target); "
            "the monolithic SPLAT/KMZ orchestration path was removed.",
            file=sys.stderr,
        )
        return 2
