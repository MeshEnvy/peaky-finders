...
| `EventSource('/api/p/{slug}/events')` | `listen('project://event', …)` — warm, viewshed ladder, seek progress |
...
**Do not** copy `init.js` line-for-line. Use it as a **behavior spec** (layer IDs, warm flow, seek UX). Prefer smaller modules under `apps/peaky/src/`.
...
7. **Seek** — candidates, scan progress, plan CRUD, convert-to-sites.
8. **Settings** — modem/environment catalogs (was `/api/home/*`).

**Parity gate** (manual + automated): open `peaky-nevada`, same workflows as today's browser session — map, viewshed warm, seek, land toggles, site edit.
...
// events (emit only): warm_status, viewshed_ladder, seek_progress, link_ready
```
...
- [ ] Feature parity: viewsheds, links, land, seek, site CRUD/import
