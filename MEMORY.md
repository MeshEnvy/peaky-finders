...
**Eligible peaks (`peaky peaks`):** CLI writes git-tracked `peaks/` (thin rows) + `access/` (jeep/hike profiles). Default **merges** into the existing catalog (freshen nearby rows, keep peaks outside the scan); `--clean` wipes then rebuilds (operator `deny: true` rows still preserved). Progressive upserts every 10 inserts. Each candidate snaps to the highest DEM point within **500 m** (disc search, no ridge chaining). Thin peak rows store `road_loc` / `paved_loc` scalars + `compute_key`; full profiles live in `access/<slug>.yaml` with their own `compute_key`. **`access/_meta.yaml`** is SoR for pathfinding (`algo_version`, highway lists, `max_jeep_m`, sample spacing, `place_road_search_m`); **`peaks/_meta.yaml`** stamps eligibility (`max_hike_m`, slope). Stale/missing keys re-warm on serve; bump code `ACCESS_ALGO_VERSION` / `PEAK_ALGO_VERSION` (or edit meta) when algorithms change. Map: logo pins, orange jeep line + paved dot, green foot-hike line + gold park dot from `GET /api/p/{slug}/peaks` (thin) + `GET …/access/{slug}` on select. Sites warm access the same way on select. Site+peak slugs are globally unique; promoting a peak keeps its slug. Access rules: jeep-class OSM ≤ 805 m to park, 3D foot hike ≤ 805 m, max segment grade **30%** on DEM profiles; jeep route required within **32187 m**. Candidate filter runs **in parallel** (rayon; default = CPU count, override `PEAKY_PEAKS_WORKERS`). **`--stop-after N`** serial early exit for smoke tests. **Goal seek** reads the peaks catalog only (hop disc ∩ progress lens, then RF rank). No live DEM peak scan or forward-path prune. Run `peaky peaks` before seek if the catalog is empty. Leftover monolithic `peaks.yaml` is scrapped (deleted), never migrated. **PLSS:** not in Peaky — ops `tag_public_land.py` (CadNSDI → preset YAML); export runs it as prep.
