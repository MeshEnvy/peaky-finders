...
Land panel (v2): informational GDB overlays from `projects/<slug>/data/**/*.gdb` — no upload, no AOI/eligible pipeline. **Import** modal: pick GDB, preview layers, per-layer **Attributes** panel (field list, row filter on attribute values, `labelField` / `styleField`, per-value colors). Registers structured `land.sources.<id>.layers[]` entries (`name`, optional `include`/`exclude`, `label_field`, `style_field`, flat or per-value `style`) in preset. GeoJSON features carry `properties.label` and `properties.style_key`; main map uses MapLibre `match` on `style_key` when styled. Sidebar shows filter summary + legend chips. Per-layer eye toggles (`landVisible` in map state). Edit/delete sources via row actions. Cache digest includes filters + label/style fields (`serve/land.py`, `serve/land_import.py`).
