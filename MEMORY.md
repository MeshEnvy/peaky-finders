...
Sites panel: **+ Site** (manual add) and **Import** (KML/KMZ Point placemarks). **Tag in view** bulk-adds/removes tags on sites in the current map viewport (`POST …/sites/tags/bulk`). Open/closed state persists per project in `localStorage` (`peaky.map.v1.<slug>`). Sidebar **In view** toggle filters the site list to the current map viewport (persists in map state). Row actions: eye (site visibility), droplet (viewshed), trash (delete). Multi-select tag filters with **all/any** mode (default **all** = intersect); hide active filter tags from row subtitles. Import flow: file → `POST …/sites/import/preview` → in-modal MapLibre preview + scrollable point list with per-row **Import** toggle; points within **100 m** of an existing site default skipped (gray on map); optional **In view** filter → tags → `POST …/sites/import` with filtered `points[]` (`serve/kml_import.py`, `import_sites_to_preset`).
