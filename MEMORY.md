...
All map points live under **`sites:`** (slug → entry with `name`, `loc: [lat, lon]`, `type`, optional `tags: [str, …]`).
...
`tags` are freeform lowercase labels (`eip`, `slpt`, …) for UI filtering; not used by build/RF.
...
| `serve` | Local web UI — Web Awesome dark theme (`wa-dark`, Font Awesome icons); project selector at `PEAKY_HOME`; toolbar **gear** opens settings modal (**Simulation** \| **Modems** \| **Environments**): `$PEAKY_HOME` catalogs + global simulation defaults; on project maps simulation tab shows effective merged values with per-field override highlight/reset, gear shows `radius · px`. MapLibre map with RF viewshed overlays (Street/USGS Topo/Satellite, 3D terrain on tilt). Floating map tools in MapLibre **top-right** nav cluster (solid bg, same style as zoom/compass): basemap layers menu, **Sites** / **Goals** toggles open left slide-out panel on the matching tab (**Sites** \| **Goals** tabs inside panel): tag filter chips (**all** \| discovered tags) hide non-matching sites on map/list/viewsheds/links; per-row show/hide (hides marker, viewshed, and link lines touching that entity), per-site viewshed toggle, delete (YAML + manual link scrub for sites; viewshed cache kept on disk), **+ Site** opens Add site modal (name, optional tag chips / new tags, pasteable `lat, lng` → `POST …/sites` `{name,lat,lon,tags?}`); **+ Goal** map-click placement → create panel → `POST …/goals`. Right detail panel on map/list click (coords, PLSS, links, viewshed toggle, **tags** with + / × via `PATCH …/sites/<slug>` `{tags}`); **Edit** opens draft editor with lat/lon inputs, map crosshair pick, comma-separated paste/copy, coordinate **history** (each move ≥25 m pushes prior coords as hidden; per-row show/copy/delete to compare candidates on map + 256 px viewshed preview), live PLSS + link prefetch; **Save** PATCHes `config.yaml` and warms full viewshed; **Cancel** resets draft. Goals may promote to `installed`/`planned` sites (fields copied, goal removed). Toolbar **Viewshed** opacity slider (0–100%) global. **Site links** / **Goal links** checkboxes toggle link layers. Compass resets north-up 2D and fits sites + goals (30 km padding). Per-browser map prefs in `localStorage` (`peaky.map.v1.<slug>`): camera, basemap, link toggles, opacity, hidden sites/goals, active tag filter, per-site viewshed visibility. |
...
| HTTP layer | — | Waitress WSGI: `serve/` package (`serve.cli`, `serve.app`); progressive APIs over shared pipeline |
| Site links | Build mesh `links` KML + footprint mutual coverage | On-demand viewshed mutual coverage via `serve/links.py` + `link_jobs` warm; in-memory cache keyed by link fingerprint (coords/sim/manual — not config mtime); GET is cache-or-manual-pending (no GPKG I/O); warm reads footprints in parallel then SSE `links` |

Map UI prefs (camera, basemap, link toggles, opacity, hidden sites/goals, active tag filter, per-site viewshed visibility) persist in browser `localStorage` per slug. **Settings** gear modal (landing + project toolbar): global modem/environment CRUD; simulation tab edits global defaults on landing or **project** effective values on map (`GET/PATCH /api/p/<slug>/simulation` with `overrides[]` + `reset`); override fields styled warning with per-field **Reset**. Saves re-warm viewsheds only when radius/raster actually change (`reloadViewshedsForSimChange`). Map warms viewsheds with bounded concurrency (6); queued jobs poll meta as SSE fallback. SSE `hello` (incl. auto-reconnect) re-`POST` warms pending sites (idempotent reconcile — no server event replay). Agent rule: `.cursor/rules/serve-web-ui.mdc`; skill: `peaky-serve`.
...
| `sites` | All map points — `installed` / `planned` / `suggested` (repeaters + viewsheds) and `goal` (attractors); optional `tags: [str, …]` |
