...
Entry: `peaky_finders.peaky_cli:main`. **`serve`**: stdlib `HTTPServer`, `--reload` polls `peaky_finders` + `serve_static/` (on by default via `./peaky serve`), Docker publishes `PEAKY_SERVE_PORT` (default 8080). UI in `serve_html.py`; assets at `/static/` (`app.css`, `project-map.js`) plus root favicons/manifest in `serve_static/`. APIs: `GET/POST /api/p/<slug>/sites`; `DELETE /api/p/<slug>/sites/<site>` removes site + scrubs manual `links:` pairs (`serve_sites.py`); `GET/POST /api/p/<slug>/goals`; `DELETE /api/p/<slug>/goals/<goal>` (`serve_goals.py`); `GET /api/p/<slug>/goal-links`; prefetch/create helpers unchanged; viewshed + site-link GET routes unchanged.
