...
| Ops | Public-land site tags + FO export live in ops: `peaky_home/scripts/tag_public_land.py`, `export_blm_fo_packet.py`. **Fleet CLI** is leaf `envybot/` (`./envybot pull` / `onboard`); nevada YAML is the book. **Do not** put passwords or keypairs in git-tracked `config.yaml`. → `ops/initiatives/peaky-fleet-management.md` |
| Reference preset | `peaky-nevada/config.yaml` (standalone project repo) |
| Releases | [`CHANGELOG.md`](CHANGELOG.md) + `./scripts/changelog.sh`; tag `v*` → GitHub Release binaries ([`docs/change-management.md`](docs/change-management.md)). EnvyOS distro may mirror `peaky-<ver>.zip` when `peaky=` is pinned in envyos `ENVYOS_VERSIONS`. |
