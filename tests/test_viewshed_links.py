...
def test_write_site_links_mutual_overlap(tmp_path: Path) -> None:
    ...
    for slug, poly in (
        ...
        _write_coverage_gpkg(splats / slug / COVERAGE_GPKG_NAME, poly)

    out = tmp_path / "links.kml"
    gpkg_a = splats / "a" / COVERAGE_GPKG_NAME
    gpkg_b = splats / "b" / COVERAGE_GPKG_NAME
...
def test_write_site_links_single_site_no_pairs(tmp_path: Path) -> None:
    splats = tmp_path / "splats"
    _write_coverage_gpkg(splats / "only" / COVERAGE_GPKG_NAME, box(-115.03, 39.00, -115.01, 39.02))
    out = tmp_path / "links.kml"
    only_gpkg = splats / "only" / COVERAGE_GPKG_NAME
    assert not write_site_links_kml(
