...
- **Jeep difficulty is road class, not grade** — jeep easy/medium/difficult/extreme now follows OSM highway + tracktype (worst meaningful stretch). A 33% DEM spike on a grade-2 dirt road is no longer extreme. OSM `grade4` is difficult; only `grade5` is extreme.
- **Hike difficulty calibrated from walked peaks** — extreme = Razorback-class (≥250 m gain and ≥18% avg). Difficult = Goldfield-class (≥80 m gain and ≥15% avg). Medium = short scramble or modest effort (max ≥15%, avg ≥8%, or gain ≥40 m). A 200 m roadside bump is medium, not extreme. Peak sheet badge label is **hike**, not “overall”.
