...
from peaky_finders.core.viewshed.raster import bbox_rotation_normalized, pixel_to_lat_lon
from peaky_finders.core.viewshed.kml_geom import orient_for_kml
from peaky_finders.core.preset.model import ViewshedPolygonStyle, DEFAULT_VIEWSHED_POLYGON_STYLE
...
def inject_peaky_polygon_kml_style(
    ...
    *,
    style_id: str,
    spec: ViewshedPolygonStyle,
    gx_draw_order: int | None = None,
) -> None:
    """Replace GDAL default placemark styling using preset ``ViewshedPolygonStyle`` (matches bundle_build rules)."""
    try:
...
def write_coverage_polygons(
    *,
    ...
    out_gpkg: Path,
    out_kml: Path,
    polygon_style: ViewshedPolygonStyle | None = None,
) -> bool:
    ...
    ``polygon_style`` defaults to :data:`DEFAULT_VIEWSHED_POLYGON_STYLE`; preset overrides via
    ...
    style = polygon_style if polygon_style is not None else DEFAULT_VIEWSHED_POLYGON_STYLE
