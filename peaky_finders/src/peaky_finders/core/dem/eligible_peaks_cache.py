"""Disk cache of Skadi binned peaks over eligible land."""

from __future__ import annotations

import hashlib
import json
import time
from collections.abc import Callable
from pathlib import Path

from shapely.geometry.base import BaseGeometry

from peaky_finders.core.dem.peaks import skadi_binned_peaks_in_polygon
from peaky_finders.core.dem.skadi_tiles import skadi_tile_set_fingerprint
from peaky_finders.core.preset.paths import ensure_skadi_mirror_dir, resolved_preset_cache_dir

ELIGIBLE_PEAKS_FORMAT = "peaky_seek_eligible_peaks/v1"
ELIGIBLE_PEAKS_ALGO = "skadi_binned_v1"


def eligible_peaks_cache_digest(
    *,
    eligible_digest: str,
    bin_size_m: float,
    eligible_ll: BaseGeometry,
) -> str:
    minx, miny, maxx, maxy = eligible_ll.bounds
    dem_fp = skadi_tile_set_fingerprint(minx, miny, maxx, maxy)
    body = "\n".join(
        (
            ELIGIBLE_PEAKS_FORMAT,
            ELIGIBLE_PEAKS_ALGO,
            str(eligible_digest),
            f"{float(bin_size_m):.3f}",
            dem_fp,
        )
    )
    return hashlib.sha256(body.encode("utf-8")).hexdigest()[:16]


def resolved_eligible_peaks_cache_dir(preset_path: Path) -> Path:
    return resolved_preset_cache_dir(preset_path) / "seek" / "eligible_peaks"


def eligible_peaks_cache_path(preset_path: Path, digest: str) -> Path:
    return resolved_eligible_peaks_cache_dir(preset_path) / f"{digest}.json"


def _peaks_to_json(peaks_llz: list[tuple[float, float, float]]) -> list[dict[str, float]]:
    return [{"lon": float(lon), "lat": float(lat), "elev_m": float(elev_m)} for lon, lat, elev_m in peaks_llz]


def _peaks_from_json(raw: list[dict]) -> list[tuple[float, float, float]]:
    out: list[tuple[float, float, float]] = []
    for row in raw:
        out.append((float(row["lon"]), float(row["lat"]), float(row["elev_m"])))
    return out


def read_eligible_peaks_cache(path: Path) -> list[tuple[float, float, float]] | None:
    p = Path(path).expanduser().resolve()
    if not p.is_file():
        return None
    try:
        body = json.loads(p.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError, TypeError, ValueError):
        return None
    if body.get("format") != ELIGIBLE_PEAKS_FORMAT or body.get("algo") != ELIGIBLE_PEAKS_ALGO:
        return None
    peaks = body.get("peaks")
    if not isinstance(peaks, list):
        return None
    try:
        return _peaks_from_json(peaks)
    except (KeyError, TypeError, ValueError):
        return None


def write_eligible_peaks_cache(
    path: Path,
    *,
    digest: str,
    eligible_digest: str,
    bin_size_m: float,
    eligible_ll: BaseGeometry,
    peaks_llz: list[tuple[float, float, float]],
) -> Path:
    p = Path(path).expanduser().resolve()
    p.parent.mkdir(parents=True, exist_ok=True)
    minx, miny, maxx, maxy = eligible_ll.bounds
    body = {
        "format": ELIGIBLE_PEAKS_FORMAT,
        "algo": ELIGIBLE_PEAKS_ALGO,
        "digest": digest,
        "eligible_digest": eligible_digest,
        "bin_size_m": float(bin_size_m),
        "dem_fingerprint": skadi_tile_set_fingerprint(minx, miny, maxx, maxy),
        "n_peaks": len(peaks_llz),
        "peaks": _peaks_to_json(peaks_llz),
    }
    p.write_text(json.dumps(body, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return p


def load_or_build_eligible_peaks(
    preset_path: Path,
    *,
    eligible_digest: str,
    eligible_ll: BaseGeometry,
    bin_size_m: float,
    max_workers: int = 1,
    verbose: bool = False,
    tile_progress: Callable[[int, int], None] | None = None,
) -> tuple[list[tuple[float, float, float]], dict[str, object]]:
    digest = eligible_peaks_cache_digest(
        eligible_digest=eligible_digest,
        bin_size_m=bin_size_m,
        eligible_ll=eligible_ll,
    )
    cache_path = eligible_peaks_cache_path(preset_path, digest)
    cached = read_eligible_peaks_cache(cache_path)
    if cached is not None:
        if verbose:
            print(f"seek peaks: cache hit {cache_path.name} ({len(cached)} peak(s))", flush=True)
        return cached, {"cache": "hit", "build_ms": None, "n_peaks": len(cached)}

    mirror = ensure_skadi_mirror_dir()
    if verbose:
        print(f"seek peaks: building eligible peak cache {digest}", flush=True)
    t0 = time.perf_counter()
    peaks = skadi_binned_peaks_in_polygon(
        eligible_ll,
        mirror,
        bin_size_m=bin_size_m,
        max_workers=max_workers,
        verbose=verbose,
        log_prefix="seek peaks: ",
        tile_progress=tile_progress,
    )
    build_ms = int((time.perf_counter() - t0) * 1000)
    write_eligible_peaks_cache(
        cache_path,
        digest=digest,
        eligible_digest=eligible_digest,
        bin_size_m=bin_size_m,
        eligible_ll=eligible_ll,
        peaks_llz=peaks,
    )
    return peaks, {"cache": "build", "build_ms": build_ms, "n_peaks": len(peaks)}
