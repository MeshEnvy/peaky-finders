...
def test_preset_yaml_rt_cache_skips_reread(tmp_path: Path) -> None:
    preset_path = _copy_minimal_preset(tmp_path)

    def add_site(slug: str) -> None:
        def mutator(_yaml_rt, root: dict) -> None:
            root.setdefault("sites", {})[slug] = {
                "name": slug,
                "loc": [39.0, -117.0],
            }
    ...
    assert elapsed_ms < 500, f"cached writes too slow: {elapsed_ms:.0f}ms"
    preset = load_preset(preset_path)
    assert "cached-a" in preset.sites
    assert "cached-4" in preset.sites
