...
from peaky_finders.plss_fetch import loc_stamp, read_plss_loc_cache, write_plss_loc_cache
from peaky_finders.serve_plss import (
...
def test_ensure_plss_for_coords_cache_hit(tmp_path: Path) -> None:
    ...
    with patch("peaky_finders.serve_plss.plss_for_point") as mock_fetch:
...
def test_ensure_plss_for_coords_network_miss(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("peaky_finders.serve_plss.plss_for_point", fake_plss)
    result = ensure_plss_for_coords(project_dir, lat, lon)
