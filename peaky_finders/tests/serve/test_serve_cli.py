...
from peaky_finders.core.plss.fetch import loc_stamp, write_plss_loc_cache
from peaky_finders.serve.app import _serialize_project_sites, make_serve_wsgi_app
from peaky_finders.serve.cli import (
...
def test_serve_child_argv_uses_no_reload() -> None:
    ...
    assert argv[1:3] == ["-m", "peaky_finders.serve.cli"]
...
def test_probe_connect_host_maps_wildcard() -> None:
    from peaky_finders.serve.cli import _probe_connect_host, _serve_public_url
...
def test_run_serve_reload_uses_supervisor(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("peaky_finders.serve.cli._supervise_serve_reload", _fake_supervisor)
    monkeypatch.setattr("peaky_finders.serve.cli._run_serve_blocking", _fake_blocking)
    monkeypatch.delenv(SERVE_RELOAD_CHILD_ENV, raising=False)
...
def test_api_project_sites_json_with_metadata(tmp_path: Path) -> None:
    projects_dir = tmp_path / "projects"
    projects_dir.mkdir()
    scaffold_project("rich", parent=projects_dir)
    config_path = projects_dir / "rich" / "config.yaml"
...
def test_post_project_site_with_multiple_sites(tmp_path: Path) -> None:
    projects_dir = tmp_path / "projects"
    project_dir = projects_dir / "nevada-like"
    project_dir.mkdir(parents=True)
    ...
    from peaky_finders.core.preset import write_preset_document
    ...
    write_preset_document(
        config_path,
        {
            ...
            "sites": {
                ...
                "bridge": {"name": "Bridge", "loc": [39.9, -119.3]},
            },
            "links": [],
        },
    )
...
def test_delete_project_site(tmp_path: Path) -> None:
    ...
    scaffold_project("mesh-demo", parent=projects_dir)
    preset_path = projects_dir / "mesh-demo" / "config.yaml"
    from peaky_finders.serve.sites import append_planned_site_to_preset
...
def test_delete_project_goal_route_removed(tmp_path: Path) -> None:
    projects_dir = tmp_path / "projects"
    projects_dir.mkdir()
    scaffold_project("mesh-demo", parent=projects_dir)
    ...
    try:
        conn = HTTPConnection(host, port, timeout=2)
        conn.request("DELETE", "/api/p/mesh-demo/goals/bridge")
        resp = conn.getresponse()
        assert resp.status == 404
    finally:
        server.shutdown()
...
def test_run_serve_exits_on_keyboard_interrupt(monkeypatch) -> None:
    ...
    monkeypatch.setattr("peaky_finders.serve.cli.waitress_serve", _interrupt)
