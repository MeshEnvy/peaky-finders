...
impl DemMosaic {
    ...
    pub fn ensure_tiles(
        ...
        if let Some(mirror) = dem_mirror {
            mirror.ensure_batch(&missing, crate::dem_mirror::PRIORITY_DEM_BLOCKING)?;
        }
        let loaded = Self::load_mirror_from_disk(mirror_root, &missing, verbose, dem_mirror.is_none())?;
    ...
    /// Load tiles already present on disk (no mirror worker blocking).
    pub fn from_mirror_on_disk(mirror_root: &Path, tile_names: &[String]) -> Result<Self> {
        Self::load_mirror_from_disk(mirror_root, tile_names, false, false)
    }
    ...
    pub fn load_mirror(
        ...
        if let Some(mirror) = dem_mirror {
            mirror.ensure_batch(tile_names, crate::dem_mirror::PRIORITY_DEM_BLOCKING)?;
        }
        Self::load_mirror_from_disk(mirror_root, tile_names, verbose, dem_mirror.is_none())
    }

    /// Decompress tiles already on disk (or sync-fetch when ``sync_fetch`` and no mirror worker).
    fn load_mirror_from_disk(
        mirror_root: &Path,
        tile_names: &[String],
        verbose: bool,
        sync_fetch: bool,
        ...
        let loaded: Vec<(TileCoord, DemTile)> = tile_names
            .par_iter()
            .map(|name| {
                let path = if sync_fetch {
                    ensure_mirror_tile(mirror_root, name, verbose)
                        .with_context(|| format!("ensure mirror tile {name}"))?
                } else {
                    mirror_root.join(name)
                };
                if verbose {
    ...
    /// Approximate native DEM cell size (m) at `(lat, lon)` — min of N–S / E–W spacing.
    /// Falls back to ~30 m (Skadi 1″) if the tile is not loaded.
    pub fn native_step_m(&self, lat: f64, lon: f64) -> f64 {
        let key = tile_coord_for_lat_lon(lat, lon);
        let Some(tile) = self.tiles.get(&key) else {
            return 30.0;
        };
        let spacing_deg = tile.spacing_deg();
        let m_lat = spacing_deg * 111_000.0;
        let m_lon = spacing_deg * 111_000.0 * lat.to_radians().cos().max(0.01);
        m_lat.min(m_lon).max(1.0)
    }

    pub fn get_tile(&self, coord: (i32, i32)) -> Option<&DemTile> {
        self.tiles.get(&coord)
    }

    /// Inject tiles for unit tests and serve integration tests.
    pub fn with_single_tile(coord: (i32, i32), tile: DemTile) -> Self {
        let mut tiles = HashMap::new();
        tiles.insert(coord, tile);
        Self { tiles }
    }

    pub fn from_tiles(tiles: HashMap<(i32, i32), DemTile>) -> Self {
        Self { tiles }
    }
}
