//! Point-to-point RF link evaluation (same knife-edge + FSPL physics as coverage).
//! Terrain sampling uses native DEM spacing; viewshed rasters may use a coarser step.
...
/// Viewshed raster ray step: `radius_m / raster_dimension` (display / footprint only).
pub fn viewshed_profile_step_m(radius_m: f64, raster_dimension: u32) -> f64 {
    let w = raster_dimension.max(1) as f64;
    (radius_m / w).max(1.0)
}
...
fn profile_step_count(distance_m: f64, step_m: f64) -> usize {
    if distance_m < 2.0 {
        return 0;
    }
    let step_m = step_m.max(1.0);
    ((distance_m / step_m).ceil() as usize)
        .max(MIN_PROFILE_STEPS)
        .max(2)
}
...
fn knife_edge_excess_loss_db_profile(
    ...
    let steps = profile_step_count(d, step_m);
    if steps < 2 {
        return 0.0;
    }
...
pub fn evaluate_link(
    ...
    // Full-res terrain: sample at native DEM spacing (not viewshed raster_dimension).
    let mid_lat = (tx_lat + rx_lat) * 0.5;
    let mid_lon = (tx_lon + rx_lon) * 0.5;
    let step_m = dem.native_step_m(mid_lat, mid_lon);
...
pub fn evaluate_mutual_site_link_strengths_parallel(
    ...
    base: &LinkContext,
) -> Vec<Option<LinkStrength>> {
    pairs
        .par_iter()
        .map(|(lat_a, lon_a, h_a, lat_b, lon_b, h_b)| {
            evaluate_mutual_site_link_strength(dem, *lat_a, *lon_a, *h_a, *lat_b, *lon_b, *h_b, base)
        })
        .collect()
}
...
pub fn evaluate_link_strengths_parallel(
    dem: &DemMosaic,
    pairs: &[(f64, f64, f64, f64)],
    ctx: &LinkContext,
) -> Vec<Option<LinkStrength>> {
    pairs
        .par_iter()
        .map(|(lat_a, lon_a, lat_b, lon_b)| {
            evaluate_link_strength(dem, *lat_a, *lon_a, *lat_b, *lon_b, ctx)
        })
        .collect()
}
