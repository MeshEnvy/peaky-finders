...
/// 1° Skadi tile names intersecting an EPSG:4326 axis-aligned bbox.
pub fn required_tile_names_for_bounds(minx: f64, miny: f64, maxx: f64, maxy: f64) -> Vec<String> {
    let lon_min_tile = minx.floor() as i32;
    let lon_max_tile = maxx.floor() as i32;
    let lat_min_tile = miny.floor() as i32;
    let lat_max_tile = maxy.floor() as i32;
    let mut out = Vec::new();
    for lat_tile in lat_min_tile..=lat_max_tile {
        for lon_tile in lon_min_tile..=lon_max_tile {
            let ns = if lat_tile >= 0 { 'N' } else { 'S' };
...
/// SW corner and NE corner (west, south, east, north) for a 1° Skadi tile name.
pub fn dem_tile_bounds(name: &str) -> Result<(f64, f64, f64, f64)> {
    let stem = name
        .trim_end_matches(".gz")
        .trim_end_matches(".hgt");
    if stem.len() < 7 {
        bail!("bad tile name {name:?}");
    }
    ...
    let south = if ns == 'N' || ns == 'n' {
        lat_d as f64
    } else {
        -(lat_d as f64)
    };
    let west = if ew == 'E' || ew == 'e' {
        lon_d as f64
    } else {
        -(lon_d as f64)
    };
    Ok((west, south, west + 1.0, south + 1.0))
}

#[cfg(test)]
mod tile_bounds_tests {
    use super::required_tile_names_for_bounds;

    #[test]
    fn dem_tile_bounds_parses_skadi_name() {
        let (w, s, e, n) = super::dem_tile_bounds("N35W116.hgt.gz").unwrap();
        assert_eq!((w, s, e, n), (-116.0, 35.0, -115.0, 36.0));
    }

    #[test]
    fn bounds_single_tile() {
        // Fully inside N39W119 (lon -119..-118, lat 39..40).
        let names = required_tile_names_for_bounds(-118.99, 39.01, -118.01, 39.99);
        assert_eq!(names, vec!["N39W119.hgt.gz"]);
    }

    #[test]
    fn bounds_two_by_two_tiles() {
        let names = required_tile_names_for_bounds(-120.0, 39.0, -118.999, 40.999);
        assert_eq!(
            names,
            vec![
                "N39W119.hgt.gz",
...
pub fn run_coverage(work_dir: &Path, verbose: bool) -> Result<()> {
    ...
    let dem = DemMosaic::load_mirror(&mirror_root, &tiles, verbose, None).context("DEM mosaic")?;
...
fn run_one_coverage(
    ...
    let profile_step_m = crate::propagate::viewshed_profile_step_m(req.radius, w);
    log(&format!(
